    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>" . $row["price"] . "</td>
                        <td>" . $row["date"] . "</td>
                        <td><form method='post'><input type='hidden' name='orderid' value='" . $row["orderid"] . "'><button class='delete-btn' type='submit' name='delete'>Complete</button></form></td>
                    </tr>";
            }
        } else {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
            ...
            if ($conn->query($sql) === TRUE) {
                echo "<script>alert('Order Terminated Successfully');</script>";
